...
const Table = React.forwardRef<
  ...
  React.HTMLAttributes<HTMLTableElement> & {
    /** Override the wrapper div (default `overflow-auto`). Use `overflow-visible` when the parent scrolls and you need sticky `thead`/`th`. */
...
>(({ className, ...props }, ref) => (
  <tr
    ref={ref}
    className={cn(
      "border-b border-border transition-colors duration-150 hover:bg-white/[0.04] data-[state=selected]:bg-white/[0.06]",
      className
    )}
    {...props}
  />
